...
def reorient_13_tile_GRID_Dataset_to_latlon_layout(gds, **kwargs):
    ...
    for key, value in list(ds_CG_r.variables.items()):
    ...
    for key, value in list(gds_r.variables.items()):
...
def reorient_13_tile_Dataset_to_latlon_layout_CG_points(ds, **kwargs):
    ...
    for key, value in list(ds_all.variables.items()):
...
def reorient_13_tile_Dataset_to_latlon_layout_UV_points(ds_U, ds_V, **kwargs):
    ...
    for key, value in list(ds_U_all.variables.items()):
    ...
    for key, value in list(ds_V_all.variables.items()):
...
def rotate_single_tile_Datasets_UV_points(ds_U, ds_V, **kwargs):
    ...
    num_vars = 0
    ki = 0
    for key, value in list(ds_U.variables.items()):
        ki = ki+1
        if key not in ds_U.coords:
            
            num_vars = num_vars+1
            
            cur_V_key = list(ds_V.variables.keys())[ki-1]
            cur_U_key = list(ds_U.variables.keys())[ki-1]
